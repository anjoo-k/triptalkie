...
//    }
	...
	public String registerCommunity(@ModelAttribute Community community, HttpSession session,
...
//		}
		community.setMember_id(loginId);
	...
	public String updateForm(@PathVariable("idx") long idx, Model model, HttpSession session,
			RedirectAttributes redirectAttributes) {
		Community community = communityService.findCommunityByIdx(idx);
		String loginId = memberService.getLoginId(session);

		if (loginId == null || !loginId.equals(community.getMemberId())) {
	...
	// 커뮤니티 글 수정 처리
	@PostMapping("/update")
	public String updateCommunity(@ModelAttribute Community community, HttpSession session,
			RedirectAttributes redirectAttributes) {
		// 1. 로그인 사용자 확인
		String loginId = memberService.getLoginId(session);

		// DB에서 원본 travelInfo 조회
		Community origin = communityService.findCommunityByIdx(community.getIdx());

		System.out.println("원본 데이터 : " + origin);

		if (!origin.getMemberId().equals(loginId)) {
			redirectAttributes.addFlashAttribute("msg", "작성자가 아닙니다.");
			return "redirect:/travel-info/detail/" + community.getIdx();
		}

		community.setMember_id(loginId);

		System.out.println("업데이트 실행 데이터: " + community);

		// 3. 업데이트
		int success = communityService.updateCommunityByIdxAndMemberId(community);
		...
		if (success > 0) {
			...
			return "redirect:/community/edit-community/" + community.getIdx();
		}
	}
