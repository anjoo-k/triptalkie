...
public class MemberImageService {
	...
	// getImageByMemberId 메서드 : member id로 이미지 경로 반환
	public String getImageUrlByMemberId(String memberId) {
	    MemberImage image = memberImageMapper.findByMemberImageById(memberId);
	    if (image != null) {
	        String uploadDir = fileStorageProperties.getUploadDir(); // "uploads"
	        return "/" + uploadDir + "/profile/" + image.getSavedName();
	    }
	...
	// deleteImageByMemberId
	// member id로 이미지 삭제 메서드
	public boolean deleteImageByMemberId(String memberId) {
	    MemberImage image = memberImageMapper.findByMemberImageById(memberId);
	    if (image != null) {
	        File file = new File(image.getFilePath());
	        if (file.exists()) file.delete();
	        return memberImageMapper.deleteByIdx(image.getIdx()) > 0;
	    }
	    return false;
	}
